...
export default function DashboardProdutividade() {
  ...
  const getMinutesDifference = (pedidoData: string, pedidoHora: string, atendimentoDataHora: string) => {
    ...
    // Ensure we only grab the time part of pedidoHora if it includes the date
    const timePart = pedidoHora.includes(' ') ? pedidoHora.split(' ')[1] : pedidoHora;

    const requestDate = new Date(`${year}-${month}-${day}T${timePart}`);
  ...
  const stats = useMemo(() => {
    ...
    data.forEach(row => {
      // Usar USUARIO_SOLICITACAO ou NM_USUARIO (fallback) como o atendente
      const userName = row.USUARIO_SOLICITACAO?.trim() || row.NM_USUARIO?.trim() || 'Desconhecido';
      ...
      if (row.HRPEDIDO) {
        const hrPart = row.HRPEDIDO.includes(' ') ? row.HRPEDIDO.split(' ')[1] : row.HRPEDIDO;
        const hr = hrPart.split(':')[0];
        if (hr) {
